...
html_dump = open("drizlylisting.html", "w+")
response = requests.get('https://drizly.com/beer/ale/ipa/c15', headers=headers)
print(response.content)
html_dump.write(str(response.content))
product_block = re.findall('<li class="CatalogResults__CatalogListItem___2qCwP">(.*?)</li>', str(response.content))
print(product_block[0])
product_url = re.findall('<a href="(.*?)"', product_block[0])
print("https://drizly.com"+product_url[0])
response = requests.get('https://drizly.com'+product_url[0], headers=headers)
print(response.content)
html_dump = open("drizlyproduct.html", "w+")
html_dump.write(str(response.content))
